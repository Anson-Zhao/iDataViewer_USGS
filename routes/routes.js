...
module.exports = function (app, passport) {
    ...
    app.get('/testGlobe', function (req, res) {
        res.render('testGlobe.ejs');
    });
...
//check if the layer name is available
    ...
    app.post('/testLocations', function (req, res) {
        let locationsData = {
        ...
        let yearData = {
            yearNumber: req.body.p_year,
            capacityNumber: req.body.p_avgcap,
            heightNumber: req.body.t_ttlh
        };

        var myStatLoc = "INSERT INTO Test_Data_Locations (latitude, longitude, measure, p_year, p_avgcap, t_ttlh) VALUES ('"+ locationsData.latitude + "','"+ locationsData.longitude +"','"+ locationsData.measure +"', '"+ yearData.yearNumber +"', '"+ yearData.capacityNumber + "', '"+ yearData.heightNumber + "');";


        con_CS.query(myStatLoc, function (err, result) {
            if (err) {
                throw err;
            } else {
                let JSONresult = JSON.stringify(result, null, "\t");
    ...
    app.get('/testLocations', function (req, res) {
        ...
        var statement = "SELECT latitude, longitude, measure, p_year, p_avgcap, t_ttlh FROM Test_Data_Locations;";
    ...
    app.get('/gradientValue', function (req,res) {
        ...
        var statement = "SELECT yearMin, yearMax, capMin, capMax, heightMin, heightMax FROM TestNumberRange;";
    ...
    app.post('/gradientValue', function (req,res) {
        res.setHeader("Acces-Control-Allow-Origin", "*");
        let colorData = {
            yearMin: req.body.yearMin,
            yearMax: req.body.yearMax,
            capMin: req.body.capMin,
            capMax: req.body.capMax,
            heightMin: req.body.heightMin,
            heightMax: req.body.heightMax
        };

        var statement = "UPDATE TestNumberRange SET yearMin = ?, yearMax = ?, capMin = ?, capMax = ?, heightMin = ?, heightMax = ?;";
        myVal = [colorData.yearMin, colorData.yearMax, colorData.capMin, colorData.capMax, colorData.heightMin, colorData.heightMax];

        con_CS.query(statement, myVal, function (err, result) {
            if (err) {
                throw err;
            } else {
                let JSONresult = JSON.stringify(result, null, "\t");
